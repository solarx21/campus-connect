...
import { sendAdmireNotification, sendMutualAdmireNotification } from '../utils/email';
...
export const voteCool = async (req: AuthRequest, res: Response) => {
  try {
...
export const admireUser = async (req: AuthRequest, res: Response) => {
  try {
    const { userId } = req.params;
    ...
    const admirer = await User.findById(admirerId);
    const admiredUser = await User.findById(userId);
    ...
    if (isMutual) {
      ...
      // Send notifications
      try {
        await sendMutualAdmireNotification(admirer.email, admiredUser.name);
        await sendMutualAdmireNotification(admiredUser.email, admirer.name);
      } catch (emailError) {
      ...
      try {
        await sendAdmireNotification(admiredUser.email);
      } catch (emailError) {
        console.error('Email sending failed:', emailError);
      }
